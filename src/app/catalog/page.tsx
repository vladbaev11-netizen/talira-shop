...
async function getProducts() {
  return client.fetch(`*[_type == "product"] | order(_createdAt desc) { name, slug, price, oldPrice, badge, inStock, mainImage, "category": category->{ name, slug } }`);
}
  ...
  return (
    <>
      ...
      <section style={{ padding: "40px 0 80px" }}>
        <div className="container-pad" style={{ maxWidth: "1320px", margin: "0 auto", padding: "0 48px" }}>
          <CatalogFilters products={JSON.parse(JSON.stringify(products))} categories={JSON.parse(JSON.stringify(categories))} />
        </div>
      </section>
