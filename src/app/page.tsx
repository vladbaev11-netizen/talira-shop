  ...
  return (
    <>
      ...
      <section style={{ padding: "72px 0", borderBottom: "1px solid var(--line)" }}>
        <div
          className="grid-hero container-pad"
          style={{
            maxWidth: "1320px",
            margin: "0 auto",
            padding: "0 48px",
        ...
        >
          ...
          <div style={{ padding: "32px 0", display: "flex", flexDirection: "column", justifyContent: "center" }}>
            ...
            >
              Меблі з підсвіткою, масажери, електроніка та б&apos;юті-рішення.
          ...
          {/* Right slider — stretches to edge */}
        ...
        <section style={{ padding: "40px 0", borderBottom: "1px solid var(--line)" }}>
          <div style={{ maxWidth: "1320px", margin: "0 auto", padding: "0 48px" }} className="container-pad">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              <h2 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "26px", fontWeight: 400 }}>
                Категорії
              </h2>
              <span style={{ fontSize: "11px", letterSpacing: ".18em", textTransform: "uppercase", color: "var(--text-dim)" }}>
                {categories.length} колекції
              </span>
            </div>
              ...
              {categories.map((cat: any, i: number) => (
                ...
                >
                  {cat.image && cat.image.asset && (
                    <Image
                      src={urlFor(cat.image).width(400).height(225).url()}
                      alt={cat.name}
                      fill
                  ...
                  <div
                    style={{
                      ...
                      right: 0,
                      padding: "16px",
                      background: "linear-gradient(180deg, transparent 0%, rgba(26,22,18,.75) 100%)",
                      color: "var(--bg)",
                    }}
                  >
                    <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "18px", fontWeight: 400, lineHeight: "1.2" }}>
                      {cat.name}
                    </h3>
                  </div>
                </Link>
      ...
      <section style={{ background: "var(--ink)", color: "var(--bg)", padding: "48px 0" }}>
        <div
          className="grid-trust container-pad"
          style={{
            maxWidth: "1320px",
            margin: "0 auto",
            padding: "0 48px",
      ...
      <section style={{ background: "var(--bg-soft)", padding: "60px 0" }}>
        <div
          className="grid-features container-pad"
          style={{
            maxWidth: "1320px",
            margin: "0 auto",
            padding: "0 48px",
...
function MiniTrust({ text }: { text: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "11px", color: "var(--text)" }}>
      <svg width="12" height="12" fill="none" stroke="var(--gold-deep)" strokeWidth="2" viewBox="0 0 24 24">
...
function TrustBlock({ num, label }: { num: string; label: string }) {
  return (
    <div>
      ...
      <div style={{ fontSize: "10px", letterSpacing: ".2em", textTransform: "uppercase", color: "rgba(245,241,232,.7)" }}>
        {label}
      </div>
    </div>
  );
...
function Feature({ icon, title, desc, border }: { icon: string; title: string; desc: string; border?: boolean }) {
  return (
    <div style={{ textAlign: "center", padding: "0 24px", borderRight: border ? "1px solid var(--line)" : "none" }}>
      <div style={{ fontSize: "24px", marginBottom: "12px" }}>{icon}</div>
      <h4 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "18px", fontWeight: 500, marginBottom: "6px" }}>{title}</h4>
      <p style={{ fontSize: "12px", color: "var(--text)", lineHeight: "1.5" }}>{desc}</p>
    </div>
  );
}
