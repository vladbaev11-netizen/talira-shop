    ...
    <>
        ...
        <div className="container-pad" style={{ maxWidth: "1320px", margin: "0 auto", padding: "0 48px" }}>
          <div style={{ maxWidth: "700px" }}>
            ...
            <h2 className="title-section" style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 400, marginBottom: "28px" }}>
              {"Від однієї ідеї до "}<em style={{ color: "var(--gold-deep)", fontStyle: "italic", fontWeight: 300 }}>{"великого каталогу"}</em>
            </h2>
            <p style={{ color: "var(--text)", fontSize: "15px", lineHeight: "1.85", marginBottom: "18px" }}>
              <strong style={{ color: "var(--ink)", fontWeight: 500 }}>{"Talira народилась у 2020 році"}</strong>{" як спроба відповісти на просте питання: чому предмети обираються за ціною, а не за враженням?"}
            </p>
            <p style={{ color: "var(--text)", fontSize: "15px", lineHeight: "1.85", marginBottom: "18px" }}>
              {"Ми почали з кількох товарів для дому. Побачили, як змінюється простір. І зрозуміли: це треба масштабувати."}
            </p>
            <p style={{ color: "var(--text)", fontSize: "15px", lineHeight: "1.85" }}>
              {"Сьогодні — "}<strong style={{ color: "var(--ink)", fontWeight: 500 }}>{counts.categories + " категорій"}</strong>{" та "}<strong style={{ color: "var(--ink)", fontWeight: 500 }}>{counts.products + " товарів"}</strong>{" у каталозі. Від кухонного посуду до масажерів, від LED-меблів до дитячих іграшок. І ми продовжуємо рости."}
            </p>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "72px 0", borderTop: "1px solid var(--line)" }}>
        <div className="container-pad grid-trust" style={{ maxWidth: "1320px", margin: "0 auto", padding: "0 48px" }}>
          ...
          <NumBlock num={String(counts.products)} label="Товарів у каталозі" />
          <NumBlock num={String(counts.categories)} label="Категорій товарів" />
        </div>
      </section>
