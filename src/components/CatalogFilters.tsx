  ...
  return (
    <div>
      <div className="filter-tabs" style={{ marginBottom: "24px" }}>
        <button onClick={() => setActiveCategory(null)} style={tabStyle(activeCategory === null)}>Все ({products.length})</button>
        {categories.map((cat) => (
          <button key={cat.slug.current} onClick={() => setActiveCategory(activeCategory === cat.slug.current ? null : cat.slug.current)} style={tabStyle(activeCategory === cat.slug.current)}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "32px", paddingBottom: "20px", borderBottom: "1px solid var(--line-soft)", flexWrap: "wrap", gap: "16px" }}>
        <div style={{ display: "flex", gap: "16px", alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <select value={sort} onChange={(e) => setSort(e.target.value as SortOption)} style={{ background: "transparent", border: "none", borderBottom: "1px solid var(--line)", color: "var(--ink)", fontFamily: "'Inter', sans-serif", fontSize: "13px", fontWeight: 500, cursor: "pointer", padding: "6px 24px 6px 4px", outline: "none" }}>
          ...
          <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", color: "var(--text)", cursor: "pointer" }}>
            <input type="checkbox" checked={onlyInStock} onChange={(e) => setOnlyInStock(e.target.checked)} style={{ accentColor: "var(--gold-deep)" }} /> В наявності
          </label>
          <label style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", color: "var(--text)", cursor: "pointer" }}>
            <input type="checkbox" checked={onlySale} onChange={(e) => setOnlySale(e.target.checked)} style={{ accentColor: "var(--gold-deep)" }} /> Зі знижкою
          </label>
        </div>
      ...
      {filtered.length > 0 ? (
        <div className="grid-4">{filtered.map((product) => (<ProductCard key={product.slug.current} product={product} />))}</div>
      ) : (
        <div style={{ textAlign: "center", padding: "80px 0", color: "var(--text-dim)" }}>
          ...
          <p style={{ fontSize: "14px" }}>
            ...
            <button onClick={() => { setActiveCategory(null); setOnlyInStock(false); setOnlySale(false); }} style={{ background: "none", border: "none", borderBottom: "1px solid var(--gold-deep)", color: "var(--gold-deep)", cursor: "pointer", fontSize: "14px", fontFamily: "inherit", padding: 0 }}>
              скинути всі фільтри
            </button>
          </p>
        </div>
