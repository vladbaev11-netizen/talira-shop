...
export default function Header() {
  ...
  return (
    <>
      <style>{`
        .nav-link { color: var(--ink-soft); font-size: 11px; letter-spacing: .22em; text-transform: uppercase; font-weight: 500; padding: 8px 16px; transition: all .25s; text-decoration: none; }
      ...
      {/* Marquee top bar */}
      <div style={{ background: "var(--ink)", color: "var(--bg)", padding: "9px 0", fontSize: "10px", letterSpacing: ".15em", textTransform: "uppercase", overflow: "hidden", whiteSpace: "nowrap" }}>
        <div style={{ display: "inline-flex", animation: "marquee 30s linear infinite" }}>
          <span style={{ padding: "0 40px" }}>{"Безкоштовна доставка від 2000 ₴"}</span>
          <span style={{ padding: "0 40px", color: "var(--gold-soft)" }}>{"·"}</span>
          <span style={{ padding: "0 40px" }}>{"Оплата при отриманні"}</span>
          <span style={{ padding: "0 40px", color: "var(--gold-soft)" }}>{"·"}</span>
          <span style={{ padding: "0 40px" }}>{"Швидка відправка замовлень"}</span>
          <span style={{ padding: "0 40px", color: "var(--gold-soft)" }}>{"·"}</span>
          <span style={{ padding: "0 40px" }}>{"Гарантія на товар"}</span>
          <span style={{ padding: "0 40px", color: "var(--gold-soft)" }}>{"·"}</span>
          <span style={{ padding: "0 40px" }}>{"Страховка посилок на пошті"}</span>
          <span style={{ padding: "0 40px", color: "var(--gold-soft)" }}>{"·"}</span>
      ...
      {menuOpen && (
        <div className="mobile-only" style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "var(--bg)", zIndex: 98, padding: "120px 24px 40px", overflowY: "auto" }}>
          <nav style={{ display: "flex", flexDirection: "column" }}>
            {navItems.map((item) => (
              <Link key={item.href} href={item.href} onClick={() => setMenuOpen(false)}
                style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: "28px", fontWeight: 400, padding: "20px 0", borderBottom: "1px solid var(--line-soft)", display: "flex", justifyContent: "space-between", alignItems: "center", color: pathname === item.href ? "var(--gold-deep)" : "var(--ink)" }}>
                {item.label}<span style={{ color: "var(--gold-deep)", fontSize: "18px" }}>{"→"}</span>
              </Link>
            ))}
          </nav>
          <div style={{ marginTop: "40px", paddingTop: "24px", borderTop: "1px solid var(--line-soft)" }}>
            ...
            <p style={{ fontSize: "14px", color: "var(--text)", marginBottom: "8px" }}>{"+380 63 529 41 05"}</p>
            <p style={{ fontSize: "14px", color: "var(--text)", marginBottom: "8px" }}>{"talira.com.ua@gmail.com"}</p>
            <p style={{ fontSize: "14px", color: "var(--text)" }}>{"@talira_com_ua"}</p>
          </div>
        </div>
      )}
    </>
