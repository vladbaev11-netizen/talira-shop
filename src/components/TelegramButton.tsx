    ...
    <>
        ...
        <div style={{
          ...
          <div style={{ background: "var(--ink)", padding: "20px", color: "var(--bg)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
              ...
              <button onClick={() => setIsOpen(false)} style={{ background: "none", border: "none", color: "rgba(245,241,232,.6)", cursor: "pointer", fontSize: "18px" }}>
                ✕
              </button>
            </div>
            <p style={{ fontSize: "13px", color: "rgba(245,241,232,.7)" }}>
              {"Ми на зв\u0027язку! Оберіть зручний спосіб."}
            </p>
          </div>
            ...
            {tab === "messengers" ? (
              ...
              status === "sent" ? (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                  <input
                    type="text" placeholder="Ваше ім'я *" value={name} onChange={(e) => setName(e.target.value)}
                  ...
                  />
                  <input
                    type="tel" placeholder="Телефон *" value={phone} onChange={(e) => setPhone(e.target.value)}
                    style={{ width: "100%", padding: "12px 14px", border: "1px solid var(--line)", borderRadius: "6px", background: "var(--bg-card)", fontSize: "14px", fontFamily: "'Inter', sans-serif", color: "var(--ink)", outline: "none" }}
                  />
                  <textarea
                    placeholder="Повідомлення (необов'язково)" value={message} onChange={(e) => setMessage(e.target.value)}
                  ...
                  <button
                    ...
                    style={{
                      padding: "14px", background: status === "sending" ? "var(--text-dim)" : "var(--gold-deep)",
                      color: "#fff", border: "none", borderRadius: "6px", fontSize: "13px",
                      fontWeight: 600, letterSpacing: ".15em", textTransform: "uppercase",
                      cursor: status === "sending" ? "wait" : "pointer", fontFamily: "'Inter', sans-serif",
                    }}
                  >
                    {status === "sending" ? "Надсилаємо..." : "Надіслати"}
                  </button>
                  {status === "error" && (
                    <p style={{ color: "#c0392b", fontSize: "12px", textAlign: "center" }}>{"Помилка. Спробуйте ще раз."}</p>
                  )}
                </div>
              )
            )}
      ...
      <button
        ...
        style={{
          ...
          boxShadow: "0 4px 16px rgba(0,0,0,.25)", cursor: "pointer", transition: "all .3s",
        }}
      >
        {isOpen ? (
