...
// Використовуємо старий перевірений класифікатор з reclassify.mjs
function classify(n) {
  n = n.toLowerCase();
  ...
  if (/охорон.*систем|сигналізац|датчик.*рух.*охорон|світловідбивн|відбиваюч/.test(n)) return ["bezpeka","signalizatsiyi"];
...
async function run() {
  console.log("=== ROLLBACK TO WORKING CLASSIFIER ===\n");
