// update-optdrop.mjs
// Adds descriptions, specs and benefits to opt-drop products
// Run: set SANITY_WRITE_TOKEN=your_token
//      node update-optdrop.mjs
...
const skuData = JSON.parse(fs.readFileSync("sku-fulldata-optdrop.json", "utf-8"));
...
async function run() {
  console.log("=== UPDATE OPT-DROP PRODUCTS ===");
  ...
  for (let i = 0; i < products.length; i += batchSize) {
    ...
    for (const p of batch) {
      ...
      if (blocks && blocks.length > 0) updates.description = blocks;
      ...
      if (data.benefits && data.benefits.length > 0) updates.benefits = data.benefits;
